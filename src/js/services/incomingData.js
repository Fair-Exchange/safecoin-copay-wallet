...
angular.module('copayApp.services').factory('incomingData', function($log, $state, $timeout, $ionicHistory, bitcoreSafe, $rootScope, payproService, scannerService, appConfigService, popupService, gettextCatalog) {
  ...
  root.redir = function(data) {
    ...
    function sanitizeUri(data) {
      ...
      // mobile devices, uris like safecoin://glidera
      newUri.replace('://', ':');
    ...
    function checkPrivateKey(privateKey) {
      try {
        new bitcoreSafe.PrivateKey(privateKey, 'livenet');
      } catch (err) {
        return false;
      }
      return true;
    ...
    if ((/^safecoin(cash)?:\?r=[\w+]/).exec(data)) {
      data = decodeURIComponent(data.replace(/safecoin(cash)?:\?r=/, ''));
      $state.go('tabs.send', {}, {
        'reload': true,
        'notify': $state.current.name == 'tabs.send' ? false : true
      }).then(function() {
    ...
    if (bitcoreSafe.URI.isValid(data)) {
        var coin = 'safe';
        var parsed = new bitcoreSafe.URI(data);
      ...
      // Plain Address
    } else if (bitcoreSafe.Address.isValid(data, 'livenet') || bitcoreSafe.Address.isValid(data, 'testnet')) {
      ...
      // Join
    } else if (data && data.match(/^safecoin:[0-9A-HJ-NP-Za-km-z]{70,80}$/)) {
      $state.go('tabs.home', {}, {
        'reload': true,
        'notify': $state.current.name == 'tabs.home' ? false : true
      }).then(function() {
