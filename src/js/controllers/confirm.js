...
angular.module('copayApp.controllers').controller('confirmController', function($rootScope, $scope, $interval, $filter, $timeout, $ionicScrollDelegate, gettextCatalog, walletService, platformInfo, lodash, configService, $stateParams, $window, $state, $log, profileService, bitcoreSafe, txFormatService, ongoingProcess, $ionicModal, popupService, $ionicHistory, $ionicConfig, payproService, feeService, bwcError, txConfirmNotification, externalLinkService) {
  ...
  $scope.$on("$ionicView.beforeEnter", function(event, data) {
    ...
    try {
      ...
      var message = gettextCatalog.getString('Copay only supports SafeCoin using new version numbers addresses');
    ...
    if (tx.coin && tx.coin == 'safe') tx.feeLevel = 'normal';
  ...
  function showSendMaxWarning(wallet, sendMaxInfo) {
    ...
    var msg = gettextCatalog.getString("{{fee}} will be deducted for safecoin networking fees.", {
  ...
  function setWallet(wallet, tx) {
    ...
    tx.coin = wallet.coin;
    tx.feeLevel = wallet.coin == 'safe' ? 'normal' : configFeeLevel;
    usingCustomFee = null;
  ...
  $scope.approve = function(tx, wallet, onSendStatusChange) {
    ...
    if ($scope.paymentExpired) {
      popupService.showAlert(null, gettextCatalog.getString('This safecoin payment request has expired.'));
      $scope.sendStatus = '';
      $timeout(function() {
        $scope.$apply();
      });
